...
func (oc *OutboxCreate) SetData(b []byte) *OutboxCreate {
	oc.mutation.SetData(b)
	return oc
}

// SetInsertedAt sets the "inserted_at" field.
func (oc *OutboxCreate) SetInsertedAt(t time.Time) *OutboxCreate {
	oc.mutation.SetInsertedAt(t)
	return oc
}
...
func (oc *OutboxCreate) check() error {
	...
	if _, ok := oc.mutation.Data(); !ok {
		return &ValidationError{Name: "data", err: errors.New(`ent: missing required field "Outbox.data"`)}
	}
	if _, ok := oc.mutation.InsertedAt(); !ok {
		return &ValidationError{Name: "inserted_at", err: errors.New(`ent: missing required field "Outbox.inserted_at"`)}
	}
	return nil
}
...
func (oc *OutboxCreate) createSpec() (*Outbox, *sqlgraph.CreateSpec) {
	...
	if value, ok := oc.mutation.InsertedAt(); ok {
